...
import static com.br.exemplo.util.OffsetDateTimeConverter.toOffsetDateTime;
...
public class CardDAO {
    ...
    public Card insert(final Card card) throws SQLException {
        ...
        try(var statement = getConnection().prepareStatement(sql)) {
            var i = 1;
            statement.setString(i++, card.getTitle());
            statement.setString(i++, card.getDescrption());
            statement.setLong(i, card.getBoardColumn().getId());
